...
display_screen = Tk()
display_screen.geometry("1300x800")
...
def DisplayForm():
    ...
    doctor_entry = Entry(LForm, textvariable=doctor, font=("Arial", 12),state="disabled")
    ...
    LButton.place(x=10, y=450, width=500, height=300)
    ...
    btn_back.grid(row=2, column=2, padx=10, pady=10)
...
def DisplayData():
    ...
    tree_frame.place(x=420, y=50, width=860, height=700)
...
def register():
    Database()
    if name.get() == "" or contact.get() == "" or age.get() == "" or test.get() == "" or date.get() == "" or gender.get() == "":
...
def Update():
    Database()
    if name.get() == "" or contact.get() == "" or age.get() == "" or test.get() == "" or date.get() == ""  or gender == "":
        tkMessageBox.showwarning('', 'Please Complete The Required Fields', icon="warning")
    elif not name.get().isalpha():
        tkMessageBox.showwarning('', 'Name should contain alphabets only', icon="warning")
    elif not age.get().isdigit() or not 1 <= int(age.get()) <= 100:
