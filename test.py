...
class LabTestDetails:
    def __init__(self, master):
        ...
        self.test_label = tk.Label(self.master, text="Lab Test:", font=("Arial", 14), bg="skyblue")
        self.test_label.grid(row=0, column=0, padx=10, pady=10, sticky=tk.E)

        self.test_var = tk.StringVar()
        self.test_combobox = ttk.Combobox(self.master, textvariable=self.test_var, font=("Arial", 14),
                                          values=["Urine Test", "Blood Test", "Blood Sugar Test", "Imaging Scan",
                                                  "Basic Metabolic Panel", "Thyroid Function Test", "Liver Panel Test"])
        self.test_combobox.grid(row=0, column=1, padx=10, pady=10, sticky=tk.W)

        self.price_label = tk.Label(self.master, text="Price:", font=("Arial", 14), bg="skyblue")
        self.price_label.grid(row=1, column=0, padx=10, pady=10, sticky=tk.E)

        self.price_var = tk.StringVar()
        self.price_entry = ttk.Entry(self.master, textvariable=self.price_var, font=("Arial", 14))
        self.price_entry.grid(row=1, column=1, padx=10, pady=10, sticky=tk.W)

        validate_cmd = self.master.register(self.validate_price)
        self.price_entry.config(validate="key", validatecommand=(validate_cmd, "%P"))

        self.button_frame = tk.Frame(self.master, bg="skyblue")
        self.button_frame.grid(row=2, column=1, columnspan=2, padx=10, pady=20, sticky=tk.W + tk.E)

        self.add_button = tk.Button(self.button_frame, text="Add", font=("Arial", 14), bg="green", fg="white",
                                    command=self.add)
        self.add_button.grid(row=0, column=0, padx=10, sticky=tk.W + tk.E)

        self.update_button = tk.Button(self.button_frame, text="Update", font=("Arial", 14), bg="orange", fg="white",
                                       command=self.update)
        self.update_button.grid(row=0, column=1, padx=10, sticky=tk.W + tk.E)

        self.delete_button = tk.Button(self.button_frame, text="Delete", font=("Arial", 14), bg="red", fg="white",
                                       command=self.delete)
        self.delete_button.grid(row=0, column=2, padx=10, sticky=tk.W + tk.E)

        self.back_button = tk.Button(self.button_frame, text="Back", font=("Arial", 14), bg="blue", fg="white",
                                     command=self.back)
        self.back_button.grid(row=0, column=3, padx=10, sticky=tk.W + tk.E)
